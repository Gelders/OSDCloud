...
Write-Host " [+] Making RunOnce AutopilotOOBE - Github" -ForegroundColor Cyan
$targetPat = ""
$targetPath = "C:\OSDCloud\Scripts\SetupComplete\DRIAutoPilotV5"
if (Test-Path $targetPath) {
    #RunOnce register sleutel
    $AutopilotOOBERuneOnce = "Start-Process PowerShell -ArgumentList '-NoL -ExecutionPolicy Bypass -File $OSDCloudMainFolderPath\Start-DRI-Autopilot-OOBE.ps1' -Wait"
    Set-ItemProperty -Path "HKLM:\Software\Microsoft\Windows\CurrentVersion\RunOnce" -Name "AutopilotOOBERuneOnce" -Value $AutopilotOOBERuneOnce
    Write-Host "   [+] REGISTER | RunOnce AutopilotOOBE reg-key aangemaakt." -ForegroundColor Green
}
